...
class FunSetSuite extends munit.FunSuite:
  ...
  test("intersect contains in both sets") {
    new TestSets:
      val s = intersect(x => x > 0, x => x < 100)

      assert(contains(s, 1), "Intersect 1")
      assert(contains(s, 99), "Intersect 2")
      assert(!contains(s, 100), "Intersect 3")
  }

  test("diff contains in first but not the second") {
    new TestSets:
      val s = diff(x => x > 0, x => x > 50)

      assert(contains(s, 1), "DIff 1")
      assert(contains(s, 49), "DIff 2")
